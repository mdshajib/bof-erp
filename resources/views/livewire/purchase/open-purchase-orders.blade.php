<div>
    ...
    <x-table.table>
        <x-slot name="head">
            <tr>
                <x-table.th width="25%" sortable :direction="$sorts['purchase_number'] ?? null" wire:click="sortBy('purchase_number')">{{ __('Purchase Number') }}</x-table.th>
                <x-table.th width="10%">{{ __('Order Date') }}</x-table.th>
                <x-table.th width="10%">{{ __('Gross Amount') }}</x-table.th>
                <x-table.th width="20%">{{ __('Generated By') }}</x-table.th>
                <x-table.th width="20%">{{ __('Action') }}</x-table.th>
                <x-table.th width="10%">{{ __('View') }}</x-table.th>
            </tr>
        </x-slot>
        <x-slot name="body">
            @php $i=0; @endphp
            @forelse ($orders as $order)
                ...
                <tr wire:key='{{ $order->id }}'>
                    ...
                    <td>
                        <button wire:click.prevent = "printPurchase({{ $order->id }})" class="btn btn-sm {{ $order->is_print ? 'btn-primary' : 'btn-secondary' }} me-2 px-3">
                            <i class="fas fa-print"></i>
                        </button>
                        <a href="{{route('purchase.edit', ["purchase_id" => $order->id])}}" class="btn {{ $order->price_updated ? 'btn-primary' : 'btn-secondary' }} btn-sm me-2">Edit</a>
                        @if($order->is_print and $order->price_updated)
                            <button wire:click.prevent = "confirmPurchase({{ $order->id }})" class="btn btn-secondary btn-sm me-2">Confirm</button>
                        @endif
                        <button wire:click.prevent = "purchaseConfirmDelete({{ $order->id }})" class="btn btn-danger btn-sm"><i class="fas fa-trash"></i></button>
                    </td>
                    <td>
                        <button wire:click.prevent = "OrderView({{ $order->id }})" class="btn btn-secondary btn-sm"><i class="fa fa-eye fa-color-primary"></i></button>
                    </td>
                </tr>
            @empty
                <tr>
