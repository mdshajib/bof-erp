        ...
        <x-slot name="body">
            ...
            @forelse ($orders as $order)
                ...
                <tr wire:key='{{ $order->id }}'>
                    ...
                    <td>
                            <button wire:click.prevent = "printPurchase({{ $order->id }})" class="btn btn-sm {{ $order->is_print ? 'btn-primary' : 'btn-secondary' }} me-2 px-3 mb-2">
                                <i class="fas fa-print"></i>
                            </button>
                            <a href="{{route('purchase.edit', ["purchase_id" => $order->id])}}" class="btn btn-secondary btn-sm mb-2 me-2">Edit</a>
                            @if($order->is_print and $order->price_updated)
                                <button wire:click.prevent = "confirmPurchase({{ $order->id }})" class="btn btn-secondary btn-sm me-2 mb-2">Confirm</button>
                            @endif
                            <button  class="btn btn-danger btn-sm mb-2"><i class="fas fa-trash"></i></button>
                    </td>
                    <td>
