<div>
    @section('page-title')
        {{ __('Manage Special Contact') }}
    @endsection


    @section('header')
        <x-common.header title="{{ __('Manage Special Contact') }}">
            <li class="breadcrumb-item">
                <a href="javascript: void(0);">{{ __('CRM') }}</a>
            </li>
            <li class="breadcrumb-item active">{{ __('Special Contact Management') }}</li>
        </x-common.header>
    @endsection
    ...
    <x-action-box>
        <x-slot name="left">
            <button wire:click="openNewContactModal()" type="button" class="btn waves-effect btn-primary">
                <i class="fa fa-plus me-2"></i> {{ __('New Special Contact') }}
            </button>
            @include('livewire.c-r-m._add_update_special_contact')
    ...
    <x-table.table>
        <x-slot name="head">
            <tr>
                ...
                <x-table.th>{{__('Batch No')}}</x-table.th>
